...
mongoose.connect('mongodb://goska:alamakota1@ds139251.mlab.com:39251/nodeappdatabase', {
  useMongoClient: true
});
...
kenny.save(function(err) {
  if (err) throw err;
  console.log('User success saved');
});
  ...
  return User.findOne({ username: 'Kenny_the_boy' })
    .then(function(user) {
        ...
        return user.save(function(err) {
          if (err) throw err;
          console.log('User ' + user.name + ' success actualization');
        })
    })
}
...
const updateUsername = function() {
  return User.findOneAndUpdate({ 
    ...
    function(err, user) {
      if (err) throw err;
      console.log('Actual name user is ' + user.username);
  })
}
